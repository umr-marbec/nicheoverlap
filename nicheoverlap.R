...
if (is.na(colorsN[1])==T) color<-c("#FF570A", "#4B8AEF", "#11A123", "#9D939C", "#761EA1", "#966410")
...
tr<-0:99
abA<-c( rep(0,40),runif(60,0,100) )
abB<-c( runif(40,0,100), rep(0,60))
abC<-c( rep(0,30), runif(40,0,100), rep(0,30))
trabex<-cbind(tr,abA,abB,abC)

layout(matrix(1:4,2,2,T))
layout.show(4)
NO_AB<-nicheoverlap(trabex[,1:3],colorsN=c("#FF570A", "#4B8AEF")); title(main="NO_AB")
NO_AC<-nicheoverlap(trabex[,c(1,2,4)],colorsN=c("#FF570A","#11A123")); title(main="NO_AC")
NO_BC<-nicheoverlap(trabex[,c(1,3,4)],colorsN=c("#4B8AEF","#11A123")); title(main="NO_BC")
NO_ABC<-nicheoverlap(trabex); title(main="NO_ABC") 
                    } # end of showexample
